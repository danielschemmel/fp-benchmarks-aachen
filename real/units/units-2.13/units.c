 ...
 *  Copyright (C) 1996, 1997, 1999, 2000-2007, 2009, 2011-2016
...
#define NOERROR_KEYWORD "noerror"
#define CO_NOARG -1
...
char *builtins[] = {"sin", "cos", "tan","ln", "log", "log2", "exp", 
                    "acos", "atan", "asin", "sqrt", "cuberoot", "per", 0};
...
char *logfilename=0;            /* Filename for logging */
FILE *logfile=0;                /* File for logging */
...
char *errormsg[]={"Successful completion", 
                  ...
                  "Malformed number",
                  "Unit name ends with nonzero digit without preceding '_'",
...
/* Replace all chars in badchars with a space */

void
spacechars(char *string, const char *badchars)
{
  while(*string){
    if (strchr(badchars, *string))
      *string = ' ';
    string++;
  }
}
...
/* Print string but replace two consecutive white space characters with
   one space. */
void
...
/*
  splits the line into two parts.  The first part is white space
  deliminted.  The second part is everything else.  Removes trailing
  white space from the second part.  Returned items are null if no
  parameter was found.
*/
        ...
        char *file,FILE *errfile)
{
  ...
  if (unitname[0]=='_' || unitname[strlen(unitname)-1]=='_'){
    if (errfile) fprintf(errfile,
  ...
  if (strchr(".,23456789", unitname[strlen(unitname)-1]) 
      && !hassubscript(unitname,strlen(unitname))){
    if (errfile) fprintf(errfile,
          ...
          char *unitname, int linenum, char *file,FILE *errfile)
{
  ...
  *end++=0;
  ...
  if (middle){
    *middle++=0;
    *second = removepadding(middle);
  } else
    *second = 0;
  
  *first = removepadding(start);
  return end;
}
             ...
             char *file, FILE *errfile)
{
  ...
  if (checkunitname(unitname, linenum, file, errfile))
    return E_BADFILE;
  funcname = removepadding(funcname);
  i = strlen(funcname)-2;                /* strip trailing () if present */
  if (i>0 && !strcmp(funcname+i,"()"))  
    funcname[i]=0;
  source = fnlookup(funcname, strlen(funcname));
  if (!source) {
    if (errfile){ 
      if (!strpbrk(funcname," ;][()+*/-^")) 
  ...
  if ((funcentry=fnlookup(unitname,strlen(unitname)))){
    if (flags.unitcheck && errfile)          
      fprintf(errfile,
  ...
  *end=0;
  ...
  while (looking_for_keywords) {
    ...
    for(i=0;fnkeywords[i].word;i++){
      if (!strncmp(fnkeywords[i].word, unitdef, strlen(fnkeywords[i].word))){
           ...
           int linenum, char *file,FILE *errfile)
{
  ...
  *end=0;
  ...
  tabpt = 0;
  start = unitdef;
   if (!strncmp(start,NOERROR_KEYWORD, strlen(NOERROR_KEYWORD)) && 
       strchr(WHITE,start[strlen(NOERROR_KEYWORD)])) {
    noerror = 1;
    start = removepadding(start+strlen(NOERROR_KEYWORD));
  }
  while (1) {
    if (tabpt>=tablealloc){
      tablealloc+=20;
    ...
    if (start==end || !strchr(WHITE,*end)){
      if (!strchr(WHITE, *start)) {
        if (strlen(start)>15) start[15]=0;
        if (errfile) fprintf(errfile, 
...
newalias(char *unitname, char *unitdef,int linenum, char *file,FILE *errfile)
{
  ...
  if ((aliasentry=aliaslookup(unitname))){   /* duplicate alias */
    ...
    if (!lastalias){
      firstalias = aliasentry;
      lastalias = firstalias;
    } else {
      lastalias->next = aliasentry;
      lastalias = aliasentry;
    }
    aliasentry->next = 0;
  }
...
checkvar(char *name, char *list)
{
  ...
  if (!name)
    return 2;
  listitem = strtok(list,WHITE);
  while (listitem){
    if (!strcmp(name, listitem))
      return 1;
    listitem = strtok(0,WHITE);
  }
  return 0;
}
          ...
          int *unitcount, int *prefixcount, int *funccount, int depth)
{
   ...
   while (!feof(unitfile)) {
      if (!fgetslong(&line, &linebufsize, unitfile, &linenum)) 
        break;
      if (linenum==1 && 0==strncmp(UTF8MARKER, line, strlen(UTF8MARKER))){ 
        int i;
...
lookupunit(char *unit,int prefixok)
{
   ...
   if (strwidth(unit)>2 && unit[strlen(unit) - 1] == 's') {
      copy = dupstr(unit);
...
parsenumformat()
{
  ...
  if (format_types == NULL){
    ...
    sprintf(testbuf, "%a", 2.0);
    if (!strncmp(testbuf,two,strlen(two)+1)) 
...
/* round a number to the displayed precision; indicate in hasnondigits
   if a number will contain any character other than the digits 0-9 in
  ...
  if (strpbrk(unitstr, "+-"))   /* show sums and differences of units */
    logprintf(" (%s)", unitstr);   /* in parens */
     ...
     /* no space or asterisk or numerator (3|8 in instead of 3 * 1|8 in) */
         ...
         && strncmp("1|", unitstr, 2) == 0 && isfract(unitstr)
...
#   define MAXPOWERTOCHECK 4
    ...
    for(i=0;i<MAXPOWERTOCHECK;i++){
      unittext[strlen(unittext)-1] = '0'+i;
...
completeunits(char *text, int state)
{
  ...
  while (checktype != CU_DONE){
    if (checktype == CU_ALIAS){
      while(curalias){
        if (checkmatch(text, curalias->name))
    ...
    while (checktype == CU_FUNC){
      ...
      if (!curfunc) 
        ...
        if (checkmatch(text,curfunc->name))
    ...
    while (checktype == CU_PREFIX){
      ...
      if (!curprefix)
        ...
        if (checkmatch(text, curprefix->name))
    ...
    while (checktype == CU_UNITS){    
      ...
      if (unitprefix){
        if (checkmatch(text+unitprefix->len, curunit->name)){
      ...
      else if (checkmatch(text,curunit->name)) 
...
getprogramname(char *path)
{
  ...
  if ((p = strrchr(path, '.')) && isexe(p))
    proglen -= 4;
  programname = (char *) mymalloc(proglen + 1, "(getprogramname)");
  strncpy(programname, path, proglen);
  programname[proglen] = '\0';
...
/*
  ...
  home directory for basename.  If home directory cannot be found, returns 
...
char *shortoptions = "VIUvqechSstf:o:d:mnpr1l:L:"
#ifdef READLINE
    "H:"
#endif
  ;
...
/*
   ...
   I an error is found then print an error message on stdout.  A
...
checkunitlist(char *unitstr, int promptlen)
{
  ...
  while (unitstr) {
    ...
    if (!unitstr[strspn(unitstr, WHITE)]) {  /* unitstr is blank */
      ...
      if (compareunits(unit, unit+1, ignore_dimless)){
        if (printerror){
          ...
          *(strchr(firstunitstr, UNITSEPCHAR)) = '\0';
          firstunitstr = removepadding(firstunitstr);
          unitstr = removepadding(unitstr);
          showpointer(promptlen);
...
checkallaliases(int verbose)
{
  ...
  for(aliasptr = firstalias; aliasptr; aliasptr=aliasptr->next){
    ...
    if (fnlookup(aliasptr->name, strlen(aliasptr->name)))
...
showunitlist(char *havestr, struct unittype *have, char *wantstr)
{
  ...
  wantstrlen = strlen(wantstr);

  if (flags.round) {
    /* disable unit repetition with terminal UNITSEPCHAR when rounding */
    if (wantstr[wantstrlen - 1] == UNITSEPCHAR)
      wantstr[wantstrlen - 1] = '\0';
  ...
  while (wantstr) {
    ...
    */
    if (!*wantstr)              /* coverity[alias_transfer] */
      wantstr = lastunitstr;
    ...
    if (firstunit){
      ...
      if (flags.round) {
        ...
        if (lastwantstr && *lastwantstr) {      /* more than one unit */
          lastwantstr = removepadding(lastwantstr);
    ...
    if (strstr(pager, "more") || strstr(pager, "less")) {
      ...
      if (*pathbuf && mksroot) {
        if (_stat(mkspager, &mksbuf)) {
          fprintf(stderr, "%s: cannot stat file '%s'. ", progname, mksbuf);
          perror((char *) NULL);
          return 0;
        }
...
remaplocale(char *filename)
{
  ...
  if (!map) {
    ...
    while(!feof(map)){
      ...
      if (!value) continue;
      *value++=0;
      value = removepadding(value);
      if (!strcmp(removepadding(name), mylocale))
...
#  endif
   ...
   if (!interactive) {
      ...
      if (wantstr) replace_minus(wantstr);
      if ((funcval = isfunction(havestr))){
        showfuncdefinition(funcval, FUNCTION);
        exit(EXIT_SUCCESS);
      }
      if (*removepadding(havestr) == '~' && 
          (funcval = isfunction(removepadding(havestr)+1))){
        showfuncdefinition(funcval, INVERSE);
        exit(EXIT_SUCCESS);
      }
      if ((alias = aliaslookup(removepadding(havestr)))){
      ...
      for (;;) {
         ...
         if (logfile && comment && isblankstr(havestr))
             fprintf(logfile, "#%s\n", comment);
         } while (isblankstr(havestr) || ishelpquery(havestr,0) ||
                  (!isfunction(havestr) 
                   && !aliaslookup(removepadding(havestr)) 
                   && (*removepadding(havestr)!='~' 
                       || !isfunction(removepadding(havestr)+1))
         ...
         do { 
           ...
           do {
             ...
             if (logfile && comment && isblankstr(wantstr)){
               fprintf(logfile, "#%s\n", comment);
               repeat = 1;
             }
